...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.Words.Intercept(interceptors...)
}
...
// A call to `Intercept(f, g, h)` equals to `words.Intercept(f(g(h())))`.
func (c *WordsClient) Intercept(interceptors ...Interceptor) {
	c.inters.Words = append(c.inters.Words, interceptors...)
}
...
func (c *WordsClient) MapCreateBulk(slice any, setFunc func(*WordsCreate, int)) *WordsCreateBulk {
	...
	if rv.Kind() != reflect.Slice {
		return &WordsCreateBulk{err: fmt.Errorf("calling to WordsClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*WordsCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *WordsClient) Query() *WordsQuery {
	return &WordsQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeWords},
...
func (c *WordsClient) Interceptors() []Interceptor {
	return c.inters.Words
}

func (c *WordsClient) mutate(ctx context.Context, m *WordsMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&WordsCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&WordsUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&WordsUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&WordsDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Words mutation op: %q", m.Op())
	}
}
